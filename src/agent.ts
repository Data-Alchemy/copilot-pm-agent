...
_Using AI-suggested types per task: ${tasksToCreate.map((t, i) => `${t.title} → **${taskTypes[i]}**`).join(', ')}_`
 ...
 /** Single-select item picker — used by /comment, /estimate, /assign, /attach (scoped to assigned user) */
 ...
 /** Multi-select item picker — used by /status and /delete (shows full board, all assignees) */
 ...
 */
 ...
 try {
 if (scope === 'board') {
 // Fetch all active items on the board — no assignee filter
